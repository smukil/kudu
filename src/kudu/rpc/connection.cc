...
Connection::Connection(ReactorThread *reactor_thread, Sockaddr remote,
    ...
    : reactor_thread_(reactor_thread),
      ...
      next_call_id_(1),
      sasl_client_(kSaslAppName, NULL),
      sasl_server_(kSaslAppName, NULL),
      negotiation_complete_(false) {
  bool ssl_enabled = reactor_thread_->reactor()->messenger()->ssl_enabled();
  if (ssl_enabled) {
  ...
  sasl_client_.set_socket(socket_.get());
  sasl_server_.set_socket(socket_.get());
}
...
Status Connection::InitSSLIfNecessary() {
  ...
  SSLSocket* ssl_socket = static_cast<SSLSocket*>(socket_.get());
