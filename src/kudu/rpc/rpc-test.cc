...
TEST_P(TestRpc, TestClientConnectionMetrics) {
  ...
  // Send a large RPC payload.
  RpcController controller;
  string s(FLAGS_rpc_max_message_size / 3, 'a');
  int idx;
  ...
  ASSERT_OK(p.SyncRequest(GenericCalculatorService::kPushTwoStringsMethodName,
  ...
  ASSERT_GT(dump_resp.outbound_connections(0).rolling_average_kbps(), 0);
